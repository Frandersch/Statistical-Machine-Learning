...
\end{align}
wobei $d$ ein Integer 
Die Verwendung von diesem Kernel fuührt dazu, dass die Entscheidungsgrenze sich ähnlich Verhält, wie als würde man zu Beginn eine Merkmalserweiterung mit Polynomen vom Grad $d$ durchführen.
Eine weitere Kernel Funktion ist der Radiale Kernel mit der Form
\begin{align}
K(x_i,x_u)=\exp\left(-\gamma ||x_i-x_u||^2\right)
\end{align}
Für diesen Kernel wird die quadrierte euklidische Distanz als Ähnlichkeitsmaß verwendet, was dazu führt, dass für diejenigen $x_i$ die näher an $x_u$ liegen, in der Entscheidungsfunktion einen größeren Einfluss haben. Es gibt noch einen Reihe wietere Kernel, die auf unterschiedlichen Ähnlichkeitsmaßen beruhen, aber eher seltener oder nur in speziellen Zusammenhänge angewendet werden.
